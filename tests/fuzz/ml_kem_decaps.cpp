...
LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
  ...
  constexpr size_t OFF_MALFORM_SKEY = 0;
  constexpr size_t OFF_MALFORM_CIPHER = OFF_MALFORM_SKEY + mk::SKEY_BYTE_LEN;
  constexpr size_t OFF_MALFORM_END = OFF_MALFORM_CIPHER + mk::CIPHER_TEXT_BYTE_LEN;
  ...
  std::array<uint8_t, mk::SKEY_BYTE_LEN> seckey{};
  std::array<uint8_t, mk::CIPHER_TEXT_BYTE_LEN> cipher{};
  std::array<uint8_t, mk::SHARED_SECRET_BYTE_LEN> ss{};

  if (data[OFF_DISCRIMINATOR] % 2 == 1) {
    // Mode A: Possibly malformed input mode
    std::copy(data + OFF_MALFORM_SKEY, data + OFF_MALFORM_CIPHER, seckey.begin());
    std::copy(data + OFF_MALFORM_CIPHER, data + OFF_MALFORM_END, cipher.begin());
  } else {
    ...
    mk::keygen(d, z, pk, seckey);
    (void)mk::encapsulate(m, pk, cipher, ss);
  }
